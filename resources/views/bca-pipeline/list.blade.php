@extends('index')
@section('title', Helper::getSiteTitle('BCA Pipeline'))
        ...
        <div class="page-title-box">
            <div class="page-title-right">
                <ol class="breadcrumb m-0">
                    ...
                    <li class="breadcrumb-item active">BCA Pipeline</li>
                </ol>
            </div>
            <h4 class="page-title">{{ !empty($business_info) && !empty($business_info->business_name) ?
                $business_info->business_name : 'BCA Pipeline' }}</h4>
        </div>
    </div>
</div>
<div class="form-wrapper bca-pipeline" id="bcaPipeline">
    <form id="bcaPipelineForm" action="{{ URL::current() }}" method="post" enctype="multipart/form-data">
        @csrf
        <div class="row">
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        ...
                        <div
                            x-data="bcaMilestoneFilter({{ json_encode($response['bcaMilestoneStages']) }}, {{ json_encode($response['nestedLoans']) }}, {currentUrl: '{{ URL::current() }}'}, {{ json_encode($response['sifMilestoneStages']) }}, {{ json_encode($response['bcaSubStages']) }}, {{ json_encode($response['sifSubStages']) }})">
                            <div class="filter-container row">
                                ...
                                <div class="col-12 mb-3 col-md-3 position-relative" x-ref="milestone-filter">
                                    ...
                                    <button @click.prevent="toggleMilestoneDropdown" class="form-select w-full">
                                        <span x-show="selectedMilestones.length === 0">Select Milestones</span>
                                        <span x-show="selectedMilestones.length > 0"
                                    ...
                                    <div x-cloak x-show="isMilestoneDropdownOpen"
                                        ...
                                        <!-- Milestone List -->
                                        <template x-for="(stage, index) in bcaMilestoneStages" :key="index">
                                            <label :class="{
                                                    ...
                                                    <div class="col-md-12 col-12">
                                                        <div class="table-responsive">
                                                            <table class="table table-bordered w-100 milestone-table"
                                                                ...
                                                                <tbody>
                                                                    <template x-if="milestone.loans.length > 0">
                                                                        <template
                                                                            x-for="(loan, i) in filterLoansByBusinessName(milestone.loans, index)"
                                                                            :key="'loan_'+i">
                                                                            <tr>
                                                                                <td class="text-start"
                                                                                    x-text="loan?.milestone"></td>
                                                                                <td class="text-start"
                                                                                    x-text="loan?.sub_milestone"></td>
                                                                                <td class="text-start"
                                                                                    x-text="loan?.advance_requested">
                                                                                </td>
                                                                                <td class="text-start"
                                                                                    x-text="loan.analyst"></td>
                                                                                <td class="text-start"
                                                                                ...
                                                                                <td class="">
                                                                                    <button type="button"
                                                                                        class="btn btn-custom text-center"
                                                                                    ...
                                                                                    <div class="">
                                                                                        @include('bca-pipeline.modal-deal-pipeline')
                                                                                    </div>
                                                                                </td>
                                                                            </tr>
                                                                        </template>
