...
class Helper
{
    ...
    public static function escapeXmlText($text)
    {
        return htmlspecialchars($text, ENT_QUOTES | ENT_XML1, 'UTF-8');
    }

    // public static function hasText($text, $paragraph)
    // {
    //     $normalizedParagraph = strtolower(str_replace('&nbsp;', ' ', $paragraph));
    //     $normalizedText = strtolower($text);

    //     return str_contains($normalizedParagraph, $normalizedText);
    // }
    ...
    public static function hasText($text, $paragraph)
    {
        ...
        // Normalize text by removing non-breaking spaces, trimming, and converting to lowercase
        $normalizedParagraph = strtolower(trim(str_replace('&nbsp;', ' ', $paragraph)));
    ...
    public static function format_currency($number, $decimals = 2, $decimalPoint = '.', $thousandsSeparator = ',', $currencySymbol = '£')
    {
        $convertedNumber = floatval($number);
        if ($convertedNumber == 0 && $number !== '0') {
            return $currencySymbol . '0.00';
        }
        return $currencySymbol . number_format($convertedNumber, $decimals, $decimalPoint, $thousandsSeparator);
    }

    public static function getUpdatedInfo(array $row, string $timestampKey, string $userIdKey): string
    {
        // Check if the timestamp is valid
        $timestamp = $row[$timestampKey] ?? null;
        $dateTime = ($timestamp && strtotime($timestamp))
            ? Carbon::parse($timestamp)->format('d-m-Y H:i')
            : '';

        // Check if the user ID is valid and retrieve user name
        $userId = $row[$userIdKey] ?? null;
        $userName = $userId
            ? User::find($userId)->user_nicename ?? 'none'
            : '';

        if (!empty($dateTime) || !empty($userName)) {
